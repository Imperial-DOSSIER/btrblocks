...
// -------------------------------------------------------------------------------------
double BP64::expectedCompressionRatio(SInteger64Stats& stats, u8 allowed_cascading_level) {
  return Integer64Scheme::expectedCompressionRatio(stats, allowed_cascading_level);
}
// -------------------------------------------------------------------------------------
u32 BP64::compress(const BIGINT* src,
                   const BITMAP* nullmap,
                   u8* dest,
                   SInteger64Stats& stats,
                   u8 allowed_cascading_level) {
  auto& col_struct = *reinterpret_cast<BP64Structure*>(dest);
  ...
  IntegerSchemePicker::compress(low.data(), nullmap, write_ptr, n, allowed_cascading_level - 1,
                                used_low, col_struct.low_scheme_code, autoScheme(), "bp64_low");
  write_ptr += used_low;
  col_struct.low_used = used_low;
  IntegerSchemePicker::compress(high.data(), nullmap, write_ptr, n, allowed_cascading_level - 1,
                                used_high, col_struct.high_scheme_code, autoScheme(), "bp64_high");
  write_ptr += used_high;
  return write_ptr - dest;
}
// -------------------------------------------------------------------------------------
namespace {
thread_local std::vector<std::vector<INTEGER>> bp64_decode_low_scratch;
thread_local std::vector<std::vector<INTEGER>> bp64_decode_high_scratch;
thread_local std::vector<std::vector<INTEGER>> bp64_gather_low_scratch;
thread_local std::vector<std::vector<INTEGER>> bp64_gather_high_scratch;
}  // namespace
// -------------------------------------------------------------------------------------
void BP64::decompress(BIGINT* dest,
                      BitmapWrapper* nullmap,
                      const u8* src,
                      u32 tuple_count,
                      u32 level) {
  const auto& col_struct = *reinterpret_cast<const BP64Structure*>(src);
  INTEGER* low =
      get_level_data(bp64_decode_low_scratch, tuple_count + SIMD_EXTRA_ELEMENTS(INTEGER), level);
  INTEGER* high =
      get_level_data(bp64_decode_high_scratch, tuple_count + SIMD_EXTRA_ELEMENTS(INTEGER), level);
...
void BP64::gather(BIGINT* dest,
  ...
  const auto& col_struct = *reinterpret_cast<const BP64Structure*>(src);
  INTEGER* low = get_level_data(bp64_gather_low_scratch, position_count, level);
  INTEGER* high = get_level_data(bp64_gather_high_scratch, position_count, level);
  IntegerSchemePicker::MyTypeWrapper::getScheme(col_struct.low_scheme_code)
...
// -------------------------------------------------------------------------------------
BIGINT BP64::lookupAt(const u8* src, BitmapWrapper* nullmap, u32 tuple_count, u32 position, u32 level) {
  BIGINT result = 0;
  this->gather(&result, src, nullmap, tuple_count, &position, 1, level);
  return result;
}
// -------------------------------------------------------------------------------------
BIGINT BP64::lookup(u32) {
  UNREACHABLE();
}
void BP64::scan(Predicate, BITMAP*, const u8*, u32) {
  UNREACHABLE();
}
// -------------------------------------------------------------------------------------
std::string BP64::fullDescription(const u8* src) {
  const auto& col_struct = *reinterpret_cast<const BP64Structure*>(src);
